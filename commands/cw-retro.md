...
**批量审计模式**（Human 说"审计最近 N 周使用"/模型升级跨期审计）：按卡——`bin/tg-transcript-slice` 切片 → 分批干净 agent 审计（纪律核对+Human 纠偏原话+TOP 候选）→ 合并走写回门入库 + 卡片修订建议交 Human；产物落 run-dir（替代单 session retro-scan.json）。
